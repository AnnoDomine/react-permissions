...
const Permission = ({
    ...
    if (permissionContext === undefined) {
        throw Error(
            "PermissionProvider not declared. Did you wrapp your app inside the Provider?"
        );
    }
    ...
    if (
        !permissionContext.config.no_permissions_needed &&
        noPermissionsNeeded
    ) {
        throw Error(
            "You did not set the 'no_permissions_needed' value to true inside your config."
        );
    }

    const [givenPermissions, setGivenPermissions] = useState(
        permissionContext.config.current.permissions
    );
    ...
    const permissionViewName =
        permissionContext.config.own_permission_name?.view ?? "view";
